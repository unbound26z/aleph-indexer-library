...
import { CLUB_PROGRAM_PROGRAM_ID } from '../../../constants'
...
export async function mockEventDAL(
  ...
  const eventDAL = createEventDAL(
    `packages/club_program/src/domain/stats/__mocks__/data/${testName}`,
  )
...
export function mockStatsStateDAL(testName: string) {
  ...
  const path = `packages/club_program/src/domain/stats/__mocks__/data/${testName}/stats_state`
...
export function mockStatsTimeSeriesDAL(testName: string) {
  ...
  const path = `packages/club_program/src/domain/stats/__mocks__/data/${testName}/stats_time_series`
  if (fs.existsSync(path)) {
    fs.rmdirSync(path, { recursive: true })
  }
